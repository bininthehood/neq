...
**금지:**
- ❌ `<span className="font-display text-accent">neq,</span>` — 텍스트 모방. 폰트가 다르고(Fraunces 계열 ↔ Instrument Serif), 색상이 다름(워드마크 오렌지 ↔ amber).
...
**색상 drift 메모:** 현 워드마크 자산은 Warm Cinema 시절 오렌지 톤(≈ #E87B35) 으로 디자인됨. 현재 액센트 amber(#C4A35A) 와 색상 불일치. 사용자 결정으로 워드마크 자체 폰트 디자인 보존 우선 — 색상 리브랜딩은 후속 작업(`design-orchestrator` 사이클). 현재 시점에서는 워드마크가 amber accent 와 별도 시각 토큰으로 존재.
