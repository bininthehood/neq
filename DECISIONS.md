...
### 24. 플랫폼 전환 전략 — 분리 모델 유지 + 점진 RNW 진화 (2026-04-30, Day 26)
- **결정:** 4 phase 단계적 진화 채택. 현 시점 코드 구조 전환 없음.
  - **Phase 1 (현재~6/18 iOS 출시):** 현 분리 모델 유지 (apps/web Next.js + apps/native Expo + packages/{design,core} 공유). 출시 일정 보호
  - **Phase 2 (출시 후 ~ 2026 Q3):** DAU 분석 + web/native 비율 측정. 신규 기능은 native 우선, web은 follow-up. **자원 배분 변경만**, 코드 구조 변경 X
  - **Phase 3 (2026 Q3 ~):** 신규 기능 빈도 ↑ + DAU 1K+ 도달 + 도구 성숙 (Solito/Tamagui v1+) 시 RNW 점진 도입 검토. **신규 컴포넌트부터** 시작, 기존 코드는 유지
  - **Phase 4 (DAU 100K+ + 팀 확장 시):** 성능 critical 영역만 부분 풀 네이티브 (Swift/Kotlin module) 검토
- **근거:**
  - **현 시점 RNW 전환 비용:** apps/web 컴포넌트 RN primitive 재작성 + Next.js/RNW 빌드 통합 + SSR/Hydration 검증 + PWA 호환성 = **~120~200h** (3~5주 풀타임). ROI 회수 2~4년 → 출시 임박 일정과 충돌
  - **풀 네이티브 전환 (대안):** 1인 개발자에게 코드 두 배 + Swift/Kotlin 별도 학습 + Expo OTA 핫픽스 포기 → 비현실적. Airbnb 같은 대규모 팀도 RN 한계로 native 회귀했지만 그건 DAU 수억 + 엔지니어 100명+ 단계
  - **분리 모델 표준성:** Notion / Linear / Netflix / Spotify 등 콘텐츠/SaaS 글로벌 표준 패턴. Neko의 OTT 도메인에 자연 부합
  - **"모바일 위주" 전략의 본의:** 코드 구조 전환이 아니라 **자원 배분 변경**이 우선. 신규 기능을 native에 먼저 투입하고 web은 follow-up하는 흐름은 코드 구조 변경 없이 가능
  - **단계 진화 패턴의 안전성:** 출시 → 데이터 → 트리거 충족 시점에 재평가하는 흐름이 1인 개발자 단계에서 가장 안전. 출시 전 큰 베팅 회피
- **영향:**
  - 현 단계 코드 변경 0 (전략만 결정)
  - Stage 4 사전 트랙 D9~D12는 **web 우선** 진행 — 현 D1 `f0982d0` / D3 `a6f0b00` 패턴 그대로 (commit 메시지에 `(web)` suffix 명시)
  - Stage 5 (5/22~) 네이티브 이관은 RN primitive 포팅에 집중 — 디자인 의사결정은 web 검증 후라 단순화
  - **Phase 2 진입 시 추가 측정 항목:** DAU의 web/native 비율, 진입 채널 분포 (web 검색 vs 앱 설치), 콘텐츠 SEO 가치
  - **Phase 3 진입 트리거 (AND):** 신규 기능 빈도 ≥ 월 5+ 신규 컴포넌트 + DAU ≥ 1K + 도구 성숙
  - **Phase 4 진입 트리거 (AND):** DAU ≥ 100K + 팀 ≥ 5명 + 성능 critical 영역 식별 + 투자 라운드 후
  - **풀 네이티브는 Phase 4에서도 부분만** — 전면 전환은 평가 제외 (Expo는 수백만 DAU 앱들도 검증된 스택)
- **참조:**
  - `_workspace/native-transition-strategy.md` §장기 플랫폼 진화
  - 본 결정의 비용 분석 + 대안 비교: D9~D12 사전 트랙 작성 시점에 사용자와 논의 (Day 26)
