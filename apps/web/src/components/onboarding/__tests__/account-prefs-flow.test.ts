 ...
 *  - Notify 단계: updateNotificationPrefs → notificationPrefs 부분 갱신. 다른 필드 보존
 *  - 5단계 모두 진행 후 prefs 가 누적되어 cold start V2 LLM 입력에 그대로 전달 가능
 *
...
describe("D4a 단계별 account_prefs 흐름", () => {
  ...
  it("Notify 단계 — updateNotificationPrefs 가 다른 필드를 보존한다", () => {
  ...
  it("5단계 진행 후 누적된 prefs 는 cold start V2 LLM 입력에 그대로 전달 가능", () => {
