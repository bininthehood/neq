      ...
      <div className="h-dvh flex flex-col items-center justify-center px-8 max-w-lg mx-auto">
        <div className="space-y-5 text-center">
          ...
          <div className="text-sm leading-relaxed text-secondary">
            ...
            <p className="mt-3 font-medium text-foreground">저장한 작품과 시청 리포트는 유지됩니다.</p>
          </div>
          {currentFavorites.length > 0 && (
            <div className="pt-2">
              <p className="text-xs mb-2 text-muted">현재 기반 작품</p>
              <div className="flex flex-wrap gap-2 justify-center">
                {currentFavorites.map((f) => (
                  <span key={f} className="px-3 py-1 text-xs bg-surface rounded-full text-secondary">{f}</span>
                ))}
              </div>
            </div>
          )}
          <div className="flex gap-3 justify-center pt-3">
            <button
              onClick={() => router.back()}
              className="px-6 py-3 text-sm font-medium active:scale-95 transition-transform bg-surface border border-border rounded-full"
            >
              돌아가기
            </button>
            <button
              onClick={() => setConfirmed(true)}
              className="px-6 py-3 text-sm font-semibold active:scale-95 transition-transform bg-danger text-foreground rounded-full"
            >
              재설정하기
            </button>
          </div>
    ...
    <div className="h-dvh flex flex-col max-w-lg mx-auto w-full">
      <div className="shrink-0 px-5 pt-8 pb-3">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-xl font-bold text-accent">취향 재설정</h1>
          <button onClick={() => router.back()} className="px-2 py-2 text-xs text-muted">취소</button>
        </div>
        ...
        <div className="flex gap-1 mt-3">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-1 flex-1 transition-colors rounded-full" style={{ background: i < selected.length ? "var(--accent)" : "var(--border)" }} />
          ))}
        </div>
          ...
          <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
            {selected.map((item) => (
              <button key={item.id} onClick={() => toggleSelect(item)} className="flex-shrink-0 relative">
                ...
                <div className="absolute -top-2 -right-2 w-7 h-7 flex items-center justify-center bg-danger rounded-full">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-24">
        {searching && <div className="text-center py-4 text-muted">검색 중...</div>}

        {showSuggestions && (
          <div>
            <div className="flex items-center justify-between mb-3 px-1">
              <p className="text-xs text-muted">이런 작품은 어떠세요?</p>
              <button onClick={fetchTrending} disabled={loadingSuggestions} className="text-xs transition-colors disabled:opacity-30 py-2 px-3 text-muted">
        ...
        {results.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => toggleSelect(item)} className="w-full flex items-center gap-3 p-3 transition-colors rounded-lg"
              ...
              {isSelected && <div className="flex-shrink-0 text-accent"><IconCheck size={18} /></div>}
            </button>
          );
        })}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 px-5 pb-6 pt-3 max-w-lg mx-auto" style={{ background: "linear-gradient(transparent, var(--bg) 30%)" }}>
        <button onClick={handleReset} disabled={selected.length < 3}
          ...
          style={{ background: selected.length >= 3 ? "var(--accent)" : "var(--surface)", color: selected.length >= 3 ? "var(--bg)" : "var(--text-muted)", cursor: selected.length >= 3 ? "pointer" : "not-allowed" }}>
